...
- 直连报协议 / 格式错误 → 需要一层**翻译代理**把 Responses ↔ Chat 互转：
  [LiteLLM](https://github.com/BerriAI/litellm) / [OpenRouter](https://openrouter.ai/) / [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)。
  起好代理后，把该 provider 的 `base_url` 改成代理地址、`model` 用代理暴露的模型名即可，其它照旧。

> 📘 DeepSeek / Kimi / GLM 实战:新版 Codex 直连这三家会 404(它们只有 Chat Completions),
> 必须经代理。完整可复现步骤见 **[接入第三方模型-代理指南.md](接入第三方模型-代理指南.md)**(用 CLIProxyAPI,含 LaunchAgent 常驻)。
